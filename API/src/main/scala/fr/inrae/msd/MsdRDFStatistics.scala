 ...
 * @author Gezim Sejdiu
 */
object MsdRDFStatistics extends Serializable {
  ...
  def run(triples: RDD[Triple]): RDD[String] = {
    Used_Classes(triples, spark).Voidify()
      ...
      .union(GraphTriples(triples, spark).Voidify())
  ...
  def voidify(stats: RDD[String], source: String, output: String): Unit = {
    ...
    val voidify = prefix.concat(src).concat(stats.collect().mkString).concat(end)
    pw.write(voidify)
...
class GraphTriples(triples: RDD[Triple], spark: SparkSession) extends Serializable {
...
object GraphTriples{
  ...
  def apply(triples: RDD[Triple], spark: SparkSession): GraphTriples = new GraphTriples(triples, spark)
}