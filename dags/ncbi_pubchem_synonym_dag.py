...
@dag(default_args=default_args,
     dag_id="pubchem_synonym",
     tags=["ingestion", "ncbi_pubchem_synonym"],
     catchup=False,
     schedule="@weekly",
...
def pubchem_synonym_taskflow():
    """
    Download pubchem synonym from https://ftp.ncbi.nlm.nih.gov/pubchem/RDF/synonym
    """

    @task(task_id="get_pubchem_synonym_info")
    def get_pubchem_synonym_info(**kwargs):
        from fonctions import http_download
        import re
        ti = kwargs['task_instance']
        version = ti.xcom_pull(key='return_value', task_ids='get_pubchem_version')
        base_url = "https://ftp.ncbi.nlm.nih.gov/pubchem/RDF/synonym/"
    ...
    get_pubchem_version >> get_pubchem_synonym_info() >> check_and_ingest()


pubchem_synonym_taskflow()
